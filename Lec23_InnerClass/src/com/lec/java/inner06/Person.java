...
public class Person {
	 ...
	 * PersonAge p = new PersonAge(); return p;
	 * 
	 * }// end createInstance /* 지역 클래스는 메소드 실행이 끝나게 되면 정의 자체가 사라지게 되는 클래스임. 메소드 내부에
	 * 정의된 지역 클래스 타입을 리턴하는 메소드는 만들 수 없다. 경우에 따라서는, 지역 클래스에 정의된 메소드를 외부에서 직접 사용하고자 하는
	 * 경우가 발생할 수도 있습니다. 그걸 가능하게 하는 방법이
	 * 
