...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		// 파일 생성 : createNewFile()
		File f2 = new File(f, TEST_FILE); // new File(dir, file) = "dir/file.asdfsad"
		...
		File f3 = new File(f, TEST_RENAME); // re_dummy.txt
		...
		if(f2.exists()) {
			...
			if(f2.renameTo(f3)){
				...
				System.out.println("파일 이름 변경 실패"); // 이미 're_dummy.txt' 존재할 시 실패
			}
		} else {
