...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception {
		...
		int month = now.getMonth() + 1; // 0이 1월임.
		int week = now.getDay(); // 0이 일요일..
		int day = now.getDate();
		int hour = now.getHours();
		int minute = now.getMinutes();
		int seconds = now.getSeconds();
		System.out.println(year + "년 " + month + "월 " + day + "일 " + hour + "시 " + minute + "분 " + seconds + "초"); // 연도는
																													// 1900
																													// 이후
																													// 경과
																													// 년도
		...
		String strNow1 = fmt1.format(now);
...
//		System.out.println(baseTime);		
		...
		Date date = new SimpleDateFormat("yyy-MM-dd HH:mm:ss.S", Locale.KOREA).parse(oldstring);
