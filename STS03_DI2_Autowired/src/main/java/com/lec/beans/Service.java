...
class RegisterService implements Service{
	...
	public RegisterService(DAO dao) {
		System.out.printf("RegisterSerivce(%s) 생성\n", dao.toString());
		this.dao = dao;
	}
...
// 기본생성자가 없는 Service 객체
class ReadService implements Service{
	DAO dao;
	
	// 기본생성자는 없다
	
	// 생성자에 @Autowired 자동주입 어노테이션
...
class ListService implements Service {
	...
	@Override
	public String toString() {
		return String.format("[ListSerivce : %s, %s]", memberDao.toString(), boardDao.toString());
	}
}//end class	
